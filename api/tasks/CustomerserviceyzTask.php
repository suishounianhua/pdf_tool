...
class CustomerserviceyzTask extends Task
{
    ...
    const PORT = '9502';
    //待回复消息统计
    const MESSAGE_STATISTICS = 'yz_messageStatistics';
    //用户key跟swoole的fd对应列表
    const TRANSFER_TABLE = 'yz_transferTable';
    //待接入消息队列
    const WAIT_MESSAGE_QUEUE = 'yz_waitMessage';
    //正在对话消息列表
    const MESSAGE_QUEUE = 'yz_message';
    //离线消息队列
    const OFFLINE_QUEUE = 'yz_offlineMessage';
    //在线的客服队列
    const CLERK_QUEUE = 'yz_clerk';
    //在线的访客队列
    const VISITOR_QUEUE = 'yz_visitor';
    //3分钟无任何会话，自动过期
    const EXPIRES = 600;
    //redis过期时间
    ...
    public function onOpen($server, $request)
    {
            ...
            in_array($request->get['type'], [self::CLERK_QUEUE, self::VISITOR_QUEUE]))
        {
            ...
            if($request->get['type'] == self::VISITOR_QUEUE && !empty($request->header['x-real-ip'])){
                $csr_session = CsryzSession::findFirst([
                    'conditions' => ['from_key' => $request->get['id'] . ":" . self::VISITOR_QUEUE]
                ]);
                if (!$csr_session) {
                    $csr_session = new CsryzSession();
                }
...
//                    $CsryzMessage = new CsryzMessage();
//                    $CsryzMessage->save($data);
                }
                ...
                $csr_session = CsryzSession::findFirst(['from_key' => $findKey]);
