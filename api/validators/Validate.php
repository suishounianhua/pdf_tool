...
class Validate extends Validation {
     ...
     *
     * 当验证类型为：keyword
     * 例：['username', '当验证类型为：keyword', '',[model,fk_id]] 表示关键字已经被使用。 model 为应用标识，fk_id 为应用关联活动id ,新增操作不需要填写
     *
     * 当验证类型为：regex
        ...
        foreach ($rules as $rule) {
            ...
            switch ($type) {
                ...
                case 'identical':
                    ...
                    break;
                case 'keyword':
                    $options['model'] = $rule[3][0] ?? null;
                    $options['fkid'] = $rule[3][1] ?? null;
                    break;
                case 'regex':
