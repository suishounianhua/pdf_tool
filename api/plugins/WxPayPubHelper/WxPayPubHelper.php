...
//        print_r($data);
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		if( $this->parameters['sub_mch_id']) {
            curl_setopt($ch,CURLOPT_SSLCERT, WxPayConf_pub::$SERVER_SSLCERT_PATH);
            curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf_pub::$SERVER_SSLKEY_PATH);
        }
        else{
...
class Wxpay_client_pub extends Common_util_pub 
{
    ...
    function getParameter($parameter = null)
    {
        if($parameter) {
	...
	function createXml()
	{
	    if(!isset($this->parameters["appid"])) {
            $this->parameters["appid"] = WxPayConf_pub::$APPID;//公众账号ID
        }
        if(!isset($this->parameters["mch_id"])){
	...
	{
	    $xml = $this->createXml();
		Di::getDefault()->get('logger')->debug('wxpay post xml:'.$xml);
...
{	
	...
	{
		$this->postXml();
		Di::getDefault()->get('logger')->debug("prepay_id response:".$this->response);
		$this->result = $this->xmlToArray($this->response);		
 ...
 * 订单查询接口
 */
class Sandbox_signkey_pub extends Wxpay_client_pub
{
    function __construct()
    {
        //设置接口链接
        $this->url = "https://api.mch.weixin.qq.com/sandboxnew/pay/getsignkey";
        //设置curl超时时间
        $this->curl_timeout = WxPayConf_pub::$CURL_TIMEOUT;
    }
...
//        {
            //检测必填参数
            if($this->parameters["partner_trade_no"] == null && $this->parameters["partner_trade_no"] == null) {
                throw new SDKRuntimeException("缺少必填参数partner_trade_no"."<br>");
            }elseif($this->parameters["openid"] == null){
                throw new SDKRuntimeException("退款申请接口中，缺少必填参数openid！"."<br>");
            }elseif($this->parameters["check_name"] == null){
                throw new SDKRuntimeException("退款申请接口中，缺少必填参数check_name！"."<br>");
            }elseif($this->parameters["amount"] == null){
                throw new SDKRuntimeException("退款申请接口中，缺少必填参数amount！"."<br>");
            }elseif($this->parameters["desc"] == null){
                throw new SDKRuntimeException("退款申请接口中，缺少必填参数desc！"."<br>");
            }
            elseif($this->parameters["spbill_create_ip"] == null){
                throw new SDKRuntimeException("退款申请接口中，缺少必填参数spbill_create_ip！"."<br>");
            }

            if(!isset($this->parameters["mch_appid"])) {
                $this->parameters["mch_appid"] = WxPayConf_pub::$APPID;//公众账号ID
            }
            if(!isset($this->parameters["mchid"])){
...
class SendRedPack_Pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        ...
        {
            //检测必填参数
            if($this->parameters["mch_billno"] == null && $this->parameters["mch_billno"] == null) {
                throw new SDKRuntimeException("缺少必填参数商户订单号mch_billno"."<br>");
            }elseif($this->parameters["send_name"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数send_name！"."<br>");
            }elseif($this->parameters["re_openid"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数re_openid！"."<br>");
            }elseif($this->parameters["total_amount"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数total_amount！"."<br>");
            }elseif($this->parameters["wishing"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数wishing！"."<br>");
            }elseif($this->parameters["act_name"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数act_name！"."<br>");
            }elseif($this->parameters["remark"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数remark！"."<br>");
            }elseif($this->parameters["act_name"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数act_name！"."<br>");
            }elseif($this->parameters["act_name"] == null){
                throw new SDKRuntimeException("发红包接口中，缺少必填参数act_name！"."<br>");
            ...
            if($this->parameters["total_num"] == null) {
                $this->parameters["total_num"] = 1;
            }

            if(!isset($this->parameters["wxappid"])) {
...
* 发送代金券接口
*/
class SendCoupon_pub extends Wxpay_client_pub{
    function __construct() 
    {
        //设置接口链接
    ...
    public function createXml(){
        if($this->parameters["coupon_stock_id"] == null && $this->parameters["coupon_stock_id"] == null) {
            throw new SDKRuntimeException("发放代金券 缺少coupon_stock_id"."<br>");
        }elseif($this->parameters["openid"] == null){
            throw new SDKRuntimeException("发放代金券，缺少必填参数openid！"."<br>");
        }
        ...
        $this->parameters["openid_count"] = 1;//openid记录数（目前支持num=1）
        if(empty($this->parameters["nonce_str"])) {
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
        }
        $this->parameters["sign"] = $this->getSign($this->parameters);//签名
