...
//        return self::$prefix.self::$TABLE_NAME;
    ...
    public static function count($parameters = null){
        ...
        if(isset($parameters['joins']) && !empty($parameters['joins'])) {
            ...
            return $result =  $builder->getQuery()->execute()->count();
        }
        else{
    ...
    * @           find_in_set  'field find_in_set' => $value
    */
            ...
            foreach ($data as $key => $value) {
                if( is_array($value) ) { //数组条件
                    if($key == 'or' || $key == 'OR') {  //或者条件
                        ...
                        if(strpos($key,' find_in_set') !== false){
                            $find_in_set_key = str_replace(' find_in_set','', $key);
                            $conditionArr[] = ' FIND_IN_SET ({'.$find_in_set_key.':array},'.$find_in_set_key.')';
                            $bindArr[ $find_in_set_key ]= $value;
                        }else{
                            // in 操作，值在范围内
